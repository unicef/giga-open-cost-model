...
class FiberCostModel:
    ...
    def _cost_of_connection(self, distance_km):
        return distance_km * self.config.capex.cost_per_km + self.config.capex.fixed_costs

    def _distance_to_costs(self, distances: List[PairwiseDistance]):
        by_school = {x.coordinate1.coordinate_id: {'school_id': x.coordinate1.coordinate_id,
                                                   'capex': self._cost_of_connection(x.distance / METERS_IN_KM)} for x in distances}
        return by_school

    def _cost_of_operation(self, _schoold):
        return self.config.opex.fixed_costs

    def compute_costs(self, distances: List[PairwiseDistance], data_space: ModelDataSpace):
        ...
        for school in data_space.school_coordinates:
            ...
            if sid in capex_costs:
                ...
                costs.append(SchoolConnectionCosts(school_id=sid,
                                                   capex=capex,
                                                   opex=opex,
                                                   technology='Fiber'))
            else:
    ...
    def run(self, data_space: ModelDataSpace, progress_bar=False) -> CostResultSpace:
        ...
        """
        conection_model = GreedyDistanceConnector(data_space.fiber_coordinates,
        ...
        return CostResultSpace(technology_results={'distances': distances},
                               cost_results=costs)
