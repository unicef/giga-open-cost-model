...
def build_sat_problem_fiber(
    ...
    #### TEST #####
    #    for fn in vertices["cfnode"]:
    #        A[(fn, "metanode")] = model.NewBoolVar("a_{0}@metanode".format(fn))
    ...
    #            A[(us,fn)] = model.NewBoolVar("a_{0}@{1}".format(us, fn))
    #        for sp in vertices["splitter"]:
    #            A[(sp,fn)] = model.NewBoolVar("a_{0}@{1}".format(sp, fn))
    #
    #    for us in vertices["uschool"]:
    #        A[(us,'metanode')] = model.NewBoolVar("a_{0}@{1}".format(us, 'metanode'))
    #    for sp in vertices["splitter"]:
    #        A[(sp,'metanode')] = model.NewBoolVar("a_{0}@{1}".format(sp, 'metanode'))
    #
    #    for us in vertices["uschool"]:
    #        for us2 in vertices["uschool"]:
    #            if us!=us2:
    #                A[(us,us2)] = model.NewBoolVar("a_{0}@{1}".format(us, us2))
    #                A[(us2,us)] = model.NewBoolVar("a_{0}@{1}".format(us2, us))
    #
    #        for sp in vertices["splitter"]:
    #            A[(us,sp)] = model.NewBoolVar("a_{0}@{1}".format(us, sp))
    #            A[(sp,us)] = model.NewBoolVar("a_{0}@{1}".format(sp, us))
    #
    #    for sp in vertices["splitter"]:
    #        for sp2 in vertices["splitter"]:
    #            if sp!=sp2:
    #                A[(sp,sp2)] = model.NewBoolVar("a_{0}@{1}".format(sp, sp2))
    #                A[(sp2,sp)] = model.NewBoolVar("a_{0}@{1}".format(sp2, sp))
    ...
    if max_cost > 0:
        ...
        for i in range(len(for_cost_vars)):
            aux = model.NewIntVar(0, for_cost_weights[i], "aux")
            model.Add(aux == for_cost_weights[i]).OnlyEnforceIf(for_cost_vars[i])
            model.Add(aux == 0).OnlyEnforceIf(for_cost_vars[i].Not())
            leq.append(aux)
        max_top = sum(for_cost_weights)
        budget_cost = model.NewIntVar(0, max_top, "Budget_cost")
        model.Add(budget_cost == sum(v for v in leq))
        model.Add(budget_cost <= new_max_cost)

        max_schools = sum(for_obj_weights)
        obj_schools = model.NewIntVar(0, max_schools, "obj_schools")
        obj_aux = []
        for i in range(len(for_obj_vars)):
            aux = model.NewIntVar(0, for_obj_weights[i], "aux")
        ...
        budget_cost = None
        obj_schools = None
        max_top = sum(for_obj_weights)
        if upper_bound > 0:
            if lower_bound > 0:
                obj_cost = model.NewIntVar(
                    lower_bound - fixed_costs, upper_bound - fixed_costs, "obj_cost"
                )
            else:
                obj_cost = model.NewIntVar(0, upper_bound - fixed_costs, "obj_cost")
        else:
            obj_cost = model.NewIntVar(0, max_top, "obj_cost")
        obj_aux = []
        for i in range(len(for_obj_vars)):
            aux = model.NewIntVar(0, for_obj_weights[i], "aux")
            model.Add(aux == for_obj_weights[i]).OnlyEnforceIf(for_obj_vars[i])
    ...
    if max_cost == 0:
        if do_hints:
            ...
            for sc in vertices["uschool"]:
                path = list(nx.shortest_path(mstG.tree, "metanode", sc))
...
def build_sat_problem_all(
    ...
    ########
    listOfConnected = ["metanode", "metaLEO", "metaCELL", "metaP2P"]
    for fn in listOfConnected:
        # Add Metanode id = metanode var = 1 always True also all metaTEchs
        X[fn] = model.NewBoolVar(f"x_{fn}")
        model.Add(
            X[fn] == True
        )  # this needs to be removed for metatechs if they have costs
    ####We start with nodes####
    for id, attributes in g.nodes(data=True):
        if id not in listOfConnected:
            label = attributes["label"]
            cost = attributes["cost"]
            ncost = attributes["ncost"]
            pkmcost = attributes["pkmcost"]
    ...
    for n1, n2, attributes in g.edges(data=True):
        ...
        if (
            ...
            if n2 in vertices["uschool"] or n2 in vertices["splitter"]:
                pkm = g.nodes[n2]["pkmcost"]
                cost = g.nodes[n2]["cost"] + g.nodes[n2]["ncost"]
                ####Parenthood#######
                P[(n2, n1)] = model.NewBoolVar("p_{0}@{1}".format(n2, n1))
                #####################
                ...
                if w > 0:
                    if max_cost == 0:
                        for_obj_vars.append(P[(n2, n1)])
                        for_obj_weights.append(w * (pkm + cost_m) + cost)
                    else:
                        for_cost_vars.append(P[(n2, n1)])
                        for_cost_weights.append(w * (pkm + cost_m) + cost)
                #############
            ...
            if (
                ...
                pkm = g.nodes[n1]["pkmcost"]
                cost = g.nodes[n1]["cost"] + g.nodes[n1]["ncost"]
                ####Parenthood#######
                P[(n1, n2)] = model.NewBoolVar("p_{0}@{1}".format(n1, n2))
                #####################
                if w > 0:
                ...
                pkm1 = g.nodes[n1]["pkmcost"]
                pkm2 = g.nodes[n2]["pkmcost"]
                cost1 = g.nodes[n1]["cost"] + g.nodes[n1]["ncost"]
                cost2 = g.nodes[n2]["cost"] + g.nodes[n2]["ncost"]
                ####Parenthood#######
                P[(n2, n1)] = model.NewBoolVar("p_{0}@{1}".format(n2, n1))
                P[(n1, n2)] = model.NewBoolVar("p_{0}@{1}".format(n1, n2))
                #####################
                ...
                if w > 0:
                    if max_cost == 0:
                        for_obj_vars.append(P[(n1, n2)])
                        for_obj_weights.append(w * (pkm1 + cost_m) + cost1)
                        for_obj_vars.append(P[(n2, n1)])
                        for_obj_weights.append(w * (pkm2 + cost_m) + cost2)
                    else:
                        for_cost_vars.append(P[(n2, n1)])
                        for_cost_weights.append(w * (pkm2 + cost_m) + cost2)
                        for_cost_vars.append(P[(n1, n2)])
                        for_cost_weights.append(w * (pkm1 + cost_m) + cost1)
                #############

                #### In this case also Pij=>!Pji ####
                model.AddBoolOr([P[(n1, n2)].Not(), P[(n2, n1)].Not()])
                #####################################
        elif n1 == "metaSAT" or n1 == "metaCELL" or n1 == "metaP2P":
            # n2 is uschool for sure
            if w > 0:
                if max_cost == 0:
                    for_obj_vars.append(P[(n2, n1)])
                    for_obj_weights.append(w)
                else:
                    for_cost_vars.append(P[(n2, n1)])
                    for_cost_weights.append(w)
    ##### We need to add edges and parenthood for metanode as well ####
    ...
    for fn in listOfConnected[1:]:
        P[(fn, "metanode")] = model.NewBoolVar("p_{0}@metanode".format(fn))
        model.Add(
            P[(fn, "metanode")] == True
        )  # this needs to be removed if metaTechs have costs
    ...
    for n1, n2 in gr.edges():
        if n1 == "metanode":
            ...
            n1 in listOfConnected
        ):  # this are metaTechs because metanode is dealt with at the beginning
            A[(n2, n1)] = model.NewBoolVar("a_{0}@{1}".format(n2, n1))
        elif (
            n2 in listOfConnected
        ):  # this are metaTechs because metanode is dealt with at the beginning
            A[(n1, n2)] = model.NewBoolVar("a_{0}@{1}".format(n1, n2))
    ##########################
    ...
    for us in vertices["uschool"]:
        ...
        for cs in vertices["cschool"]:
            if (us, cs) in P:
                clause.append(P[(us, cs)])
        for tc in listOfConnected[1:]:
            if (us, tc) in P:
                clause.append(P[(us, tc)])
    ...
    for us in vertices["uschool"]:
        ...
        for tc in listOfConnected[1:]:
            if (us, tc) in P:
                model.AddImplication(P[(us, tc)], A[(us, tc)])
    ...
    for tc in listOfConnected[1:]:
        model.Add(
            A[(tc, "metanode")] == True
        )  # need to change this if metaTechs have costs
