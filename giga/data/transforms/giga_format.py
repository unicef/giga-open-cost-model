...
def cell_towers_to_standard_format(frame: pd.DataFrame):
    ...
    t = frame.rename(columns={'Site ID': 'tower_id',
    ...
    t['outdoor'] = t['outdoor'].apply(lambda x: True if x.lower() == 'outdoor' else False)
    t['technologies'] = t['technologies'].apply(lambda x: ['3G'] if x == '3G Only' else x.split('/'))
