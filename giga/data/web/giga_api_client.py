...
DEFAULT_SCHOOL_ID_MAP = {'Brazil': 144,
                         'Rwanda': 32}
...
class GigaAPIClient:
    ...
    def _get_document_pages(self, base_url: str,
                                  start_page: int = 1,
        ...
        doc_page = start_page
        documents = []
        while (len(documents) < max_number_of_documents and # fetch until we reach target
               0 < found_in_prev_request and # or we get no documents
               found_in_prev_request == docs_per_request): # or we get less documents than desired per request
            try:
                url = f'{base_url}?page={doc_page}&size={docs_per_request}'
                r = giga_requests.get(url, self.auth_header, exception=requests.exceptions.ConnectionError, **kwargs)
    ...
    def get_schools(self, country: str, **kwargs):
        ...
        school_url = kwargs.get('school_url', 'https://uni-connect-services-dev.azurewebsites.net/api/v1/schools/country')
        country_id = self.school_id_map[country]
        base_url = f'{school_url}/{country_id}'
        schools = self._get_document_pages(base_url,
                                           docs_per_request=500_000, # fetch all schools at once
                                           document_formatter=lambda x: x['data'], # school info is found in {'data': <>}
                                           max_retries=3,
                                           **kwargs)
        return schools
