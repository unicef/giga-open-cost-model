...
class CountryDefaultsRegistration(BaseModel):
    ...
    def as_validator(defaults: Dict):
        ...
        assert "data" in defaults, "Default configuration must have a top level 'data' section"
        data_lookup_required = set(["country", "country_code", "workspace", "school_file", "fiber_file", "cellular_file"])
        for key in data_lookup_required:
            assert key in defaults["data"], f"Default configuration must have a 'data.{key}' section"
...
class CountryDefaults(BaseModel):
    ...
    def from_defaults(defaults: Dict, **kwargs):
        ...
        data = DataDefaults(
            ...
            school_file=os.path.join(data_defaults["workspace"], data_defaults["country"], data_defaults["school_file"]),
            fiber_file=os.path.join(data_defaults["workspace"], data_defaults["country"], data_defaults["fiber_file"]),
            cellular_file=os.path.join(data_defaults["workspace"], data_defaults["country"], data_defaults["cellular_file"]),
        ...
        return CountryDefaults(data=data, model_defaults=defaults["model_defaults"], **kwargs)
