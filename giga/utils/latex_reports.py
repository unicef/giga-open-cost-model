...
def generate_cost_report_old(dashboard):
    ...
    if vals_num['budget_cstr']==math.inf:
    ...
    for tech in vals_num['input_technologies']:
        latex_source += " "+tech+","
    ...
    if vals['num_all_schools']==vals_num['num_schools']:
...
def generate_infra_report(data_space):
    ...
    #doc = append_infra_report_vals(doc)
...
def append_infra_report_vals(doc, vals):
    ...
    doc.preamble.append(Command(NoEscape(r'newcommand{{\percconnknown}}{{${}$}}'.format(vals['perc_conn_known']))))
    doc.preamble.append(Command(NoEscape(r'newcommand{{\percconnunknown}}{{${}$}}'.format(vals['perc_conn_unknown']))))
    doc.preamble.append(Command(NoEscape(r'newcommand{{\percconntypeknown}}{{${}$}}'.format(vals['perc_conn_type_known']))))
    doc.preamble.append(Command(NoEscape(r'newcommand{{\percconntypeunknown}}{{${}$}}'.format(vals['perc_conn_type_unknown']))))
    doc.preamble.append(Command(NoEscape(r'newcommand{{\perceleknown}}{{${}$}}'.format(vals['perc_ele_known']))))
    doc.preamble.append(Command(NoEscape(r'newcommand{{\perceleunknown}}{{${}$}}'.format(vals['perc_ele_unknown']))))
