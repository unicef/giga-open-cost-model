...
def results_to_aggregates(results, n_years=5):
    df = results_to_table(results, n_years=n_years).drop(columns=['school_id', 'technology', 'feasible', 'reason'])
    dfm = pd.DataFrame([dict(df.mean())]).round({"opex": 1, "capex": 1, "total_cost": 1})
