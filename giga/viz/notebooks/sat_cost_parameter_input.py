...
class SATCostEstimationParameterInput:
    """
    ...
    def __init__(
        ...
        sat_solver_parameter_manager=None,
        show_map=SHOW_MAP,
    ):
        self._hashed_sheets = {}
        ...
        self.sat_solver_parameter_manager = (
            SATSolverParameterManager()
            if sat_solver_parameter_manager is None
            else sat_solver_parameter_manager
        )
        self.managers = {
            ...
            "algorithm": [self.sat_solver_parameter_manager],
        }
    ...
    def sat_parameters(self, budget: int = 0, cost_per_km: float = 0):
        return self.sat_solver_parameter_manager.get_model_parameters(
            budget=budget, cost_per_km=cost_per_km
        )
    ...
    def scenario_parameters(self, sheet_name="scenario"):
        ...
        if p["scenario_type"] == "Fiber Only":
            ...
            p.tech_config = tech_params
            return p
        elif p["scenario_type"] == "Fiber Constrained (SAT)":
            fiber_params = self.fiber_parameters()
            budget = p["cost_minimizer_config"]["budget_constraint"]
            cost_per_km = fiber_params.capex.cost_per_km
            sat_params = self.sat_parameters(budget=budget, cost_per_km=cost_per_km)
            conf = SATMinimumCostScenarioConf(
                **p, technologies=[fiber_params], sat_solver_config=sat_params
            )
            conf.cost_minimizer_config.budget_constraint = budget
            return conf
        else:
            fiber_params = self.fiber_parameters()
    ...
    def parameter_input(self):
        ...
        return VBox(
            [
                ...
                HTML(value="<hr><b>SAT Parameters</b>"),
