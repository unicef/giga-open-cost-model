...
def output_to_electricity_capex(output_table):
    ...
    return pd.DataFrame([{"Number Schools Requiring Solar": len(output_table[output_table["capex_electricity"] > 0.0]),
                          "Per School Solar Panel Costs": value_to_dollar_format(per_school_solar),
                          "Total Solar Costs": value_to_dollar_format(total_solar)}]).transpose().style.hide_columns()
...
def display_summary_table(output_space):
    ...
    cap_total = format_currency(sum(dfcap["Total Costs"]), currency="USD", locale="en_US")
    op_total = format_currency(sum(dfop["Total Annual Cost"]), currency="USD", locale="en_US")
    ...
    display(HTML(f"<b><font color='#50ba83'>Total CapEx Costs {get_space('&emsp;', 2)} {cap_total}</b><hr><hr>"))
    ...
    display(HTML(f"<b><font color='#50ba83'>Total OpEx Costs {get_space('&emsp;', 12)} {op_total}</b><hr><hr>"))
...
def plot_cost_breakdown(output_space, data_space, border):
    ...
    schools.plot(column = 'total_cost', ax=ax1, cmap='rainbow',
